...
# ---------- Helpers ----------
def get_gcal_creds():
    """Return a google.oauth2.credentials.Credentials object from encrypted session."""
    if 'gcal_credentials' not in session:
        return None
    creds_json = fernet.decrypt(session['gcal_credentials'].encode()).decode()
    return Credentials(**json.loads(creds_json))


def save_gcal_creds(creds: Credentials):
    """Encrypt and save credentials to session."""
    creds_dict = {
        'token': creds.token,
    ...
    session['gcal_credentials'] = encrypted_creds
        ...
        if title and date_str:
            ...
            if sync_to_gcal:
                ...
                if creds:
                    ...
                    sync_event_to_gcal(new_event, service)
    ...
    if creds:
        ...
        for e in filtered_gcal_events:
            ...
            if not start_str:
                continue
            try:
                start_dt = parser.isoparse(start_str).replace(tzinfo=None)
                end_dt = parser.isoparse(end_str).replace(tzinfo=None) if end_str else start_dt
            except Exception:
                continue
...
def sync_all_to_gcal():
    ...
    for event in unsynced_events:
        try:
            sync_event_to_gcal(event, service)
